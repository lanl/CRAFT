...
# Check if we have multiple drives/cases or single drive/case
if len(lu) > 1:
    print("Processing multiple drives and cases")
    # Process each drive/case combination
    for idx, row in lu.iterrows():
        DRIVE1 = row["DRIVE"]
        CASENAME = row["CASE"]
        CASE_ID = row["CASE_ID"]
        print(f"Processing drive: {DRIVE1}, case: {CASENAME}, case_id: {CASE_ID}")

        varlist = row["Varlist"].split(",")
        print(varlist)
        Nruns = int(row["N_runs"])
        print(Nruns)
        samples = row["Parameter_loc"]
        print(samples)
        timesteps = row["timestep"].split(",")
        ...
        samples_df["Model"]=list(range(1,len(samples_df)+1))
        ...
        for var in subset_varlist[1:]:
            ...
            for i in ms:
                ...
                sub1=sub1.iloc[4:,:].merge(samples_df,on="Model",how="left")
            ...
            df2.to_csv(f'data/Vars/{var}MonthlyforML_{CASE_ID}_{CASENAME}.csv')
        ...
        for var in subset_varlist[1:]:
            ...
            for i in ms:
               ...
               # sub1['Date']=pd.DatetimeIndex(sub1["Date"])
                ...
                sub1["year"]=sub1["Date"].dt.year
                df2=pd.concat([df2,sub1])
            df2.to_csv(f'data/Vars/{var}DailyforML_{CASE_ID}_{CASENAME}.csv')
        ...
        for var in subset_varlist[1:]:
            ...
            for i in ms:
               ...
               # sub1['Date']=pd.DatetimeIndex(sub1["Date"])
                ...
                sub1["year"]=sub1["Date"].dt.year
                df2=pd.concat([df2,sub1])
            df2.to_csv(f'data/Vars/{var}DailyminforML_{CASE_ID}_{CASENAME}.csv')
        ...
        for var in subset_varlist[1:]:
            ...
            for i in ms:
               ...
               # sub1['Date']=pd.DatetimeIndex(sub1["Date"])
                ...
                sub1["year"]=sub1["Date"].dt.year
                df2=pd.concat([df2,sub1])
            df2.to_csv(f'data/Vars/{var}DailymaxforML_{CASE_ID}_{CASENAME}.csv')
else:
    # Single drive/case processing (backward compatibility)
    print("Processing single drive and case")
    DRIVE1=lu["DRIVE"][0]
    print(DRIVE1)
